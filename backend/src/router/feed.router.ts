...
import { createFeedProduct, existingFeedProduct, getAllFeedProduct, updateFeedUnitSize } from "../controller/feed.controller.js";
...
router.patch("/update/:feedProductId", async (req: Request, res: Response) => {
  const { feedProductId } = req.params;
  ...
  try {
    const updated = await updateFeedUnitSize(feedProductId, unitSize);
